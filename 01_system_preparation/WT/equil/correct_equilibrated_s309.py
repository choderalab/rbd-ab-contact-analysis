...
positions_R, positions_X, positions_A, positions_B, positions_Y = list(), list(), list(), list(), list()

# Create new chains
new_chain_R = new_topology.addChain(id="R")
new_chain_X = new_topology.addChain(id="X")
new_chain_A = new_topology.addChain(id="A")
new_chain_B = new_topology.addChain(id="B")
new_chain_Y = new_topology.addChain(id="Y")

# Specify the starting residue ids for each chain
d_current_start = {"A": 1, "R": 331, "X": 1, "B": 1}
...
for res in old_topology.residues():  
    ...
    if res.name not in ['HOH', 'Na+', 'Cl-']:
        if residue_id <= 200:
            ...
            d_current_start["R"] += 1
        elif residue_id >= 201 and residue_id <= 210:
            ...
            d_current_start["X"] += 1
        elif residue_id >= 211 and residue_id <= 439:
            new_res = new_topology.addResidue(res.name, new_chain_A, id=str(d_current_start["A"]), insertionCode=res.insertionCode)
            for atom in res.atoms():
                new_atom = new_topology.addAtom(atom.name, atom.element, new_res)
                d_old_to_new[atom] = new_atom
                positions_A.append(old_positions[atom.index])
            d_current_start["A"] += 1
        elif residue_id >= 440 and residue_id <= 653:
            new_res = new_topology.addResidue(res.name, new_chain_B, id=str(d_current_start["B"]), insertionCode=res.insertionCode)
            for atom in res.atoms():
                new_atom = new_topology.addAtom(atom.name, atom.element, new_res)
                d_old_to_new[atom] = new_atom
                positions_B.append(old_positions[atom.index])
            d_current_start["B"] += 1
    else:
...
new_positions = positions_R + positions_X + positions_A + positions_B + positions_Y
