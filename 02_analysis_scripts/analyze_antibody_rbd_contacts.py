...
parser.add_argument(
    ...
    dest="traj_path",
    type=str,
    help="the location of the FAH trajectories i.e. /path/to/traj/, assuming files named as run0-clone0.xtc etc",
)
parser.add_argument(
    "-stride_clones",
    dest="stride_clones",
...
parser.add_argument(
    ...
    dest="discard_time",
    default=0,
    help="the amount of time at the start of each clone to discard, default = 0 ns"
)
...
def get_traj_list(run_number, clones, stride_clones=1):
...
def populate_dict(
    ...
    run : int
        The run number associated with the supplied trajectory
    equil_time : int
        The equilibration time (in ns) used for the system, default = 21
    discard_time : int
    ...
    for ts in u.trajectory[::frames_to_stride]:
        ...
        if time_ns < int(discard_time) + int(equil_time):
            continue
...
def write_result(
    result, project_code, mutant, n_clones, stride_clones, stride_frames
):
    ...
    for r in result:
        ...
        pickle_name = f"PROJ{project_code}_{mutant}_Abchain{r.chain_num}_distances_run_{r.run}_clone{r.clone}.pkl"
