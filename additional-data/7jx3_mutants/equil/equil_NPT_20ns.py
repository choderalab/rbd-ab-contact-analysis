...
parser = argparse.ArgumentParser(description='run 10 ns equil')
parser.add_argument('subdir', type=str, help='subdir for mutant')
parser.add_argument('sim_number', type=str, help='job id - 1')
...
amber_prmtop_file = f"../run_tleap/{args.subdir}/7jx3_s309_mutant.prmtop"
amber_inpcrd_file = f"../run_tleap/{args.subdir}/7jx3_s309_mutant.inpcrd"

# Set file names
output_prefix = f'/data/chodera/zhangi/vir/coronavirus/ab_systems/7jx3_s309_mutants/{args.subdir}/{args.sim_number}/'
...
# Add virtual bond between S309 chains (last SGs of each chain)
force = CustomBondForce('0')
force.addBond(6400, 9625, [])
system.addForce(force)

# Add virtual bond between chain A of S309 (CB of PHE106) and RBD (CE of LYS356)
force = CustomBondForce('0')
force.addBond(4933, 398, [])
system.addForce(force)
...
# Save the minimized structure as a PDB
print("Saving minimized structure as %s" % state_pdb_filename)
