...
parser.add_argument(
    ...
    type=float,
    default=3.5,
    help="the cutoff distance to use to count an interaction, default = 3.5 Å",
)
parser.add_argument(
    "-ab_chain_key",
    dest="ab_chain_key",
...
parser.add_argument(
    "-mutant", dest="mutant", type=str, help="the mutant, e.g. WT, P337A etc"
)
...
def count_percent_int(
    ...
    cutoff : float
        The cutoff at which to count an interaction
    Returns
    -------
    results_array : xarray
...
if __name__ == "__main__":
    ...
    clones_to_analyse = []
    for clone_number in range(0, 3000, file_slice_val):

        file = os.path.join(
            path_name,
            f"PROJ{project_number}_{mutant}_Abchain{ab_chain_key}_distances_clone{clone_number}.pkl",
        )
